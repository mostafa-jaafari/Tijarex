...
export default function DropDownPublicProfilMenu({ session }: { session: Session | null }) {
    ...
    const ProfileHeaderNavs = [
        ...
        { label: "cart", icon: ShoppingCart, href: "/c/cart" },
    ];
    ...
    return (
        ...
        >
            ...
            >
                ...
                >
                    {isLoadingUserInfos ? (
                        <span className='flex w-7 h-7 bg-neutral-200 rounded-full animate-pulse'/>
                    )
                    :
                    (userInfos?.profileimage || session?.user?.image) && (
                        <Image
                            ...
                            className="object-cover overflow-hidden rounded-full 
                                ring-2 border-2 border-neutral-100 ring-teal-600"
                        />
                    )}
                    <span
                        className="absolute w-2 h-2 rounded-full 
                            bg-green-500 right-0 bottom-0 border-1 
                            border-white"
                    />
                </div>
                <span className="text-sm font-medium text-neutral-700">
                    {isLoadingUserInfos ? 
                        (
                            <div>
                                <span 
                                    className="animate-pulse w-18 h-3 bg-neutral-200 
                                        flex rounded-md"
                                />
                                <span 
                                    className="animate-pulse w-30 h-3 mt-1 bg-neutral-200 
                                        flex rounded-md"
                                />
                            </div>
                        )
                        ...
                        (
                            <span className="flex flex-col items-start">
                                ...
                                <span className="text-xs text-gray-500">
                                    {userInfos?.email.slice(0, 16)}...
                                </span>
                            </span>
                        )}
                </span>
                <ChevronsUpDown size={16} className="text-neutral-700"/>
            </button>
            ...
            <div 
                className={`absolute right-0 top-14 w-64 bg-white 
                    ...
                    >
            <div className="w-full py-2 px-3 border-b border-neutral-200">
                {isLoadingUserInfos ? (
                    <div className="w-full flex items-center gap-3">
                        <span className="w-10 h-10 rounded-lg bg-neutral-300 animate-pulse"/>
                        <span className="flex flex-col gap-2">
                            <span className="w-24 h-3 bg-neutral-300 rounded-md animate-pulse"/>
                            <span className="w-20 h-3 bg-neutral-300 rounded-md animate-pulse"/>
                        </span>
                    </div>
                ) : (
                    <div className="w-full flex items-center gap-3">
                        ...
                        >
                            <Image
                                ...
                                fill
                                className="object-cover overflow-hidden rounded-full 
                                    ring-2 border-2 border-2-800 ring-teal-600"
                            />
                            <span
                    ...
                    {ProfileHeaderNavs.map((item, idx) => {
                        return (
                            <Link
                                ...
                                href={item.href.toLowerCase().replace(" ", "")}
                                className="group capitalize flex items-center gap-3 px-2 py-2 
                                    rounded-lg text-sm
                                    text-neutral-800 hover:bg-neutral-100 transition-colors">
