...
const AffiliateAvailableProductsContext = createContext<
  AffiliateAvailableProductsContextTypes | undefined
>(undefined);

export const AffiliateAvailableProductsProvider = ({
  children,
}: {
  children: ReactNode;
}) => {
  const [affiliateAvailableProductsData, setAffiliateAvailableProductsData] =
    useState<ProductType[]>([]);
  const [isLoadingAffiliateAvailableProducts, setIsLoadingAffiliateAvailableProducts] =
    useState(true);
  ...
  const fetchProducts = useCallback(async (cursor: string | null = null) => {
    ...
    try {
      const url = `/api/products/affiliate-available-products?limit=10${
        cursor ? `&lastVisibleId=${cursor}` : ""
      }`;
      ...
      setAffiliateAvailableProductsData((prev) =>
...
export function useAffiliateAvailableProducts() {
  ...
  if (!context) {
    throw new Error(
      "useAffiliateAvailableProducts must be used within an AffiliateAvailableProductsProvider"
    );
  }
  return context;
}
